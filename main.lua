-- =========================================================================
--  ZYLOHUB UI FRAMEWORK (v3.5 - DELTA CRASH-FIXED EDITION)
...
-- Hapus GUI lama jika ada agar tidak double / error
pcall(function()
    if gethui then
        local old = gethui():FindFirstChild("ZyloHub_v3_5_Master")
        if old then old:Destroy() end
    end
    local oldCore = CoreGui:FindFirstChild("ZyloHub_v3_5_Master")
    if oldCore then oldCore:Destroy() end
    local oldPlr = LocalPlayer:FindFirstChild("PlayerGui") and LocalPlayer.PlayerGui:FindFirstChild("ZyloHub_v3_5_Master")
...
-- =========================================================================
-- [3] ALGORITMA 13 TITIK TELUR (LOCKED)
-- =========================================================================
...
-- =========================================================================
-- [6B] SAFE PET INVENTORY DETECTOR & HATCH WORKER
-- =========================================================================
local function GetAllInventoryPets(): table
    local petList = {}
    pcall(function()
        local dsModule = ReplicatedStorage:FindFirstChild("Modules") and ReplicatedStorage.Modules:FindFirstChild("DataService")
        if not dsModule then return end
        local ds = require(dsModule)
        if not ds or not ds.GetData then return end
        
        local data = ds:GetData()
        if not data or not data.PetsData or not data.PetsData.AllPets then return end

        local mutRegModule = ReplicatedStorage:FindFirstChild("Data") and ReplicatedStorage.Data:FindFirstChild("PetRegistry") and ReplicatedStorage.Data.PetRegistry:FindFirstChild("PetMutationRegistry")
        local mutReg = mutRegModule and require(mutRegModule)
        ...
        for uuid, pInfo in pairs(data.PetsData.AllPets) do
            ...
            if mutReg and mutReg.EnumToPetMutation and mutEnum then
            ...
            local isFavorite = (pData.Favorite == true or pData.IsFavorite == true or pData.Locked == true or pInfo.Favorite == true)
...
local function EquipSelectedTeam(roleName: string)
    ...
    pcall(function()
        local petService = require(ReplicatedStorage.Modules.PetServices.PetsService)
        local petUtil = require(ReplicatedStorage.Modules.PetServices.PetUtilities)
        if petService and petUtil and petUtil.GetPetsSortedByAge then
            ...
            for _, act in ipairs(activePets) do
                if not selectedUUIDs[act.UUID] then
                    petService:UnequipPet(act.UUID)
...
-- =============================================================
-- [7] UI VISUAL DESIGN (SAFE PARENTING UNTUK DELTA EXECUTOR)
-- =============================================================
...
ScreenGui.Name = "ZyloHub_v3_5_Master"
ScreenGui.ResetOnSpawn = false

-- Protected Parenting Anti-Crash Delta
...
print("[ZyloHub v3.5] Clean Launch - Delta Executor Ready!")
